...
Schotter — after Georg Nees, 1968.
    ...
    A triangle grid that falls apart as it descends. The top row is perfectly ordered;
...
COLS = 12            # triangles across
ROWS = 22            # triangles down — the chaos builds over this many rows
SEED = 5959          # any integer. Same seed = same image, every time, forever.
CHAOS = 1.0           # how fast order collapses. 0 = perfect grid. 2 = rubble.
SQUARE = 40          # size of one triangle, in svg units
...
def triangle(x, y, size, angle_deg, dx, dy):
    """One triangle, rotated about its centre and nudged off its grid slot."""
    cx, cy = x + size / 2, y + size / 2
    return (
        f'  <polygon points="{x + size / 2:.2f},{y:.2f} '
        f'{x + size:.2f},{y + size:.2f} {x:.2f},{y + size:.2f}" '
...
def draw():
    ...
    for row in range(ROWS):
        ...
        for col in range(COLS):
            ...
            parts.append(triangle(x, y, SQUARE, angle, dx, dy))
...
if __name__ == "__main__":
    with open(OUTPUT, "w", encoding="utf-8") as f:
        f.write(draw())
    print(f"wrote {OUTPUT} — {COLS}x{ROWS} triangles, seed {SEED}, chaos {CHAOS}")
